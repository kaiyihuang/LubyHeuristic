...
def heurstic_wrapper(limit):
    ...
    while len(ans_set) < limit:
        ...
        if len(old_heurstic) > 0:
            ...
            ans = new_heur
        #for index, his in enumerate(heurstic_hist[1:]): #Consult previous heurstics
            #commons.union( heurstic_hist[index-1].intersection(his) )
            #commons = set(random.sample(commons, int(len(commons)/2) )) #Flatten it by half


        #new_heur = commons.union(new_heur)

        #heurstic_hist.append(new_heur) #Add it to the list
        old_heurstic = heurstic_set
        heurstic_set = ans
        #if len(heurstic_hist) > max_heurstic_hist:
            #heurstic_hist.pop()
...
def cellular_wrap(limit):
    ans_set = set()
    while len(ans_set) < (int)(limit * 2.5):
